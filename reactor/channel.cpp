...
Channel::Channel(int fd, Epoll *ep, bool isListen): m_fd(fd), m_ep(ep), m_inepoll(false), m_isListen(isListen), m_event(0), m_revent(0) {
...
void Channel::handleEvent(Socket *serversock) {
    if (m_revent & EPOLLRDHUP) { // client disconnect, some OS may not support this signal
        ...
        if (m_isListen) { // client try to connect
            ...
            Channel *clientChannel = new Channel(clientsock->fd(), m_ep, false);
